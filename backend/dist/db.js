...
async function initDb() {
    const run = (q, p) => withRetry(() => exports.db.query(q, p));
  ...
  `);
    ...
    const existingSchedule = await withRetry(() => exports.db.get('SELECT * FROM schedule LIMIT 1'));
    if (!existingSchedule) {
        await withRetry(() => exports.db.run(`
    ...
    const admin = await withRetry(() => exports.db.get('SELECT * FROM users WHERE role = $1', ['admin']));
    if (!admin) {
        const bcrypt = await Promise.resolve().then(() => __importStar(require('bcryptjs')));
        const hash = await bcrypt.hash('admin123', 10);
        await withRetry(() => exports.db.run(`INSERT INTO users (id, email, password_hash, name, role) VALUES ($1, $2, $3, $4, $5)`, ['admin-1', 'admin@zionite.online', hash, 'Admin User', 'admin']));
    }
    else if (admin.email === 'admin@zionitefm.com') {
        await withRetry(() => exports.db.run(`UPDATE users SET email = $1 WHERE id = $2`, ['admin@zionite.online', admin.id]));
    }
}
